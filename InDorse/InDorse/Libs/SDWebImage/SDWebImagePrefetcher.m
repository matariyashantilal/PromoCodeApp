...
- (void)startPrefetchingAtIndex:(NSUInteger)index
{
    ...
    [self.manager downloadWithURL:self.prefetchURLs[index] options:self.options progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished)
    {
        ...
        if (image)
        {
            DLog(@"Prefetched %d out of %d", self.finishedCount, self.prefetchURLs.count);
        }
        else
        {
...
- (void)reportStatus
{
    ...
    DLog(@"Finished prefetching (%d successful, %d skipped, timeElasped %.2f)", total - self.skippedCount, self.skippedCount, CFAbsoluteTimeGetCurrent() - self.startedTime);
}
