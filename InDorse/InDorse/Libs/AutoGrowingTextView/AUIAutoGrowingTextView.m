...
@property (strong, nonatomic) NSLayoutConstraint *bottomConstratint;
...
-(void) commonInit
{
    ...
    for (NSLayoutConstraint *constraint in self.constraints) {
        if (constraint.firstAttribute == NSLayoutAttributeBottom) {
            self.bottomConstratint = constraint;
            break;
        }
    }
...
-(void) handleLayoutWithAutoLayouts
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"ChatViewHeightChanged" object:self];

    self.heightConstraint.constant = intrinsicSize.height;
    self.bottomConstratint.constant = self.bottomConstratint.constant + intrinsicSize.height;
    DLog(@"%f",intrinsicSize.height);
}
