...
+ (MBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated closeButton:(BOOL)closeButton
{
    //if(closeButton)
    //{
	...
	MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:view];
	[view addSubview:hud];
	//[hud show:animated :closeButton];
    [hud show:animated btnCloseFlag:closeButton];
	return MB_AUTORELEASE(hud);
}
...
- (void)show:(BOOL)animated  btnCloseFlag:(BOOL)btnCloseFlag
{
    
   // if(self.isCloseDisplay)
    
    btnClose.hidden = !btnCloseFlag;
...
- (void)setupLabels {
    ...
    if(IS_IPHONE5){
    btnClose.frame = CGRectMake(self.bounds.origin.x+213, self.bounds.size.width - 88, 20, 20);
    }
    else{
        btnClose.frame = CGRectMake(self.bounds.origin.x+210, self.bounds.size.width - 130, 20, 20);
    }
    ...
    btnClose.titleLabel.font = self.labelFont;
...
{
   // //DebugLog(@"btnClose Pressed");
    //=>    Post notification
    //[[NSNotificationCenter defaultCenter] postNotificationName:kNotification_CloseProgress object:self];
}
- (void)updateIndicators {
...
- (void)drawRect:(CGRect)rect {
    ...
    if([self.detailsLabelText length] > 0)
    {
        if(IS_IPHONE5)
        {
            ...
            btnClose.frame = CGRectMake(self.bounds.origin.x+210, self.bounds.size.width - 100, 20, 20);
        }
        else{
            btnClose.frame = CGRectMake(self.bounds.origin.x+210, self.bounds.size.width - 140, 20, 20);
        }
    }
